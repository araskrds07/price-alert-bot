...
Environment:
  ...
  TRIAL_DAYS       free days per user, counted from their own first message (0 = off)
  TRIAL_UNTIL      YYYY-MM-DD: the whole bot stops after this date (customer installs)
  TRIAL_CONTACT    shown when a trial ends, e.g. @yourname
...
TRIAL_DAYS = int(os.environ.get("TRIAL_DAYS") or 0)        # per user: free days from their first message (0 = off)
TRIAL_UNTIL = (os.environ.get("TRIAL_UNTIL") or "").strip()  # whole bot: YYYY-MM-DD hard stop (customer installs)
TRIAL_CONTACT = os.environ.get("TRIAL_CONTACT") or ""      # e.g. @yourname — shown when a trial ends
WARN_BEFORE_DAYS = 2
...
RATE_LIMIT = int(os.environ.get("RATE_LIMIT") or 40)   # max messages/taps per chat per minute
...
def price_view(q):
    ...
    text += tl_equiv(q)
    if is_delayed(q["key"]):
        text += "\n<i>Borsa, altın ve döviz fiyatları ~15 dk gecikmeli.</i>"
...
def summary_kb(state, chat_id):
    ...
    return ikb([[(f"🔔 Günlük özeti aç · her gün {SUMMARY_TIME}", "ss|on")]])
...
def admin_view(state):
    ...
    text = (f"🛠 <b>Yönetim paneli</b> · v{VERSION}\n\n"
            ...
            + (f"⏳ Deneme: <b>{TRIAL_DAYS} gün</b> (kişi başı)\n" if TRIAL_DAYS else "")
            + (f"⏳ Bot son kullanma: <b>{TRIAL_UNTIL}</b>\n" if TRIAL_UNTIL else "")
            + (f"♾ Süresiz kullanıcı: <b>{sum(1 for u in users.values() if u.get('no_trial'))}</b>\n" if TRIAL_DAYS else "")
...
def subscribe_summary(state, chat_id, on, hhmm=None):
    ...
    return (f"🔔 Günlük özet açık.\nGönderim saati: <b>{hhmm}</b> (her gün)\nKapatmak için: /ozet kapat")
...
COMMANDS = {
    ...
    "id": "id", "uzat": "extend", "bitir": "endtrial", "kullanicilar": "users", "kullanıcılar": "users",
    "admin": "admin", "duyuru": "broadcast", "izinver": "allow", "izinkaldir": "disallow", "ozelmod": "pmode",
}
...
def handle_text(state, ctx, text):
    ...
    if not text.startswith("/"):
        ...
        if parsed:
            s, kind, v, rep = parsed
            return send(chat_id, *add_alert(state, chat_id, s, kind, v, rep))
        if " " not in text and normalize_symbol(text) and len(text) <= 20:
            return reply_price(chat_id, text)
        return send(chat_id, "Ne yapmamı istersin? 🙂\n"
                             "• <code>BTC</code> → fiyat\n• <code>BTC 70000</code> → alarm\n"
                             "• <code>altın 7000</code> · <code>dolar 50</code> · <code>THYAO 400</code>\n"
                             "Ya da aşağıdaki butonları kullan.", MAIN_KB)
    ...
    if cmd in ("admin", "broadcast", "allow", "disallow", "pmode", "extend", "endtrial", "users"):
        ...
        if cmd == "extend":
            ...
            if not parts or not re.fullmatch(r"-?\d+", parts[0]):
                return send(chat_id, "Kullanım:\n<code>/uzat 12345678</code> — süreyi baştan başlatır\n"
                                     "<code>/uzat 12345678 sinirsiz</code> — süresiz yapar")
            cid = parts[0]
            u = state["users"].setdefault(cid, {"name": cid, "type": "private", "since": int(time.time())})
            u.pop("trial_warn_sent", None)
            u.pop("trial_end_sent", None)
            u.pop("blocked", None)
            if len(parts) > 1 and parts[1].lower() in ("sinirsiz", "sınırsız", "limitsiz", "unlimited"):
                u["no_trial"] = True
                send(int(cid), "✅ Erişimin açıldı, süre sınırı yok. Alarmların yine çalışıyor.")
                return send(chat_id, f"♾ {cid} artık süresiz.")
            u.pop("no_trial", None)
            u["since"] = int(time.time())
            send(int(cid), f"✅ Süren uzatıldı: {TRIAL_DAYS} gün daha. Alarmların yine çalışıyor.")
            return send(chat_id, f"⏳ {cid} için süre baştan başladı ({TRIAL_DAYS} gün).")
        if cmd == "endtrial":
            if not re.fullmatch(r"-?\d+", args.strip()):
                return send(chat_id, "Kullanım: <code>/bitir 12345678</code> — o kişiye botu kapatır")
            cid = args.strip()
            u = state["users"].setdefault(cid, {"name": cid, "type": "private", "since": int(time.time())})
            u["blocked"] = True
            u.pop("no_trial", None)
            send(int(cid), f"⏳ <b>Erişimin kapandı.</b>{_contact()}")
            return send(chat_id, f"🚫 {cid} kapatıldı. Geri açmak için: <code>/uzat {cid}</code>")
        if cmd == "users":
            rows = []
            for cid, u in sorted(state["users"].items(), key=lambda kv: kv[1].get("since", 0), reverse=True)[:30]:
                started = datetime.fromtimestamp(u.get("since", 0), LOCAL_TZ).strftime("%d.%m %H:%M")
                ...
                rows.append(f"• <b>{e(u.get('name', cid))}</b>\n  <code>{cid}</code> · başlangıç {started} · "
                            f"{durum} · {n} alarm")
            if not rows:
                return send(chat_id, "Henüz kullanıcı yok.")
            return send(chat_id, "👥 <b>Kullanıcılar</b> (en yeniden eskiye)\n\n" + "\n".join(rows)
...
# ---------------------------------------------------------------- trial
def _contact():
    return f"\nDevam etmek için: {e(TRIAL_CONTACT)}" if TRIAL_CONTACT else ""
...
def trial_notices(state):
    ...
    for cid, u in list(state["users"].items()):
        ...
        if left <= 0 and not u.get("trial_end_sent"):
            ...
            send(int(cid), f"⏳ <b>Deneme süren doldu.</b>\nAlarmların artık çalışmıyor.{_contact()}")
        elif 0 < left <= WARN_BEFORE_DAYS and not u.get("trial_warn_sent"):
            u["trial_warn_sent"] = True
            d = max(1, int(round(left)))
            send(int(cid), f"⏳ Ücretsiz denemenin bitmesine <b>{d} gün</b> kaldı. "
                           f"Alarmların o zamana kadar çalışmaya devam eder.{_contact()}")
        ...
        try:
            if "callback_query" in u:
                ...
                if chat.get("id") is None:
                    continue
                if not allowed(state, chat["id"], user.get("id")) or rate_limited(chat["id"]):
                    tg("answerCallbackQuery", callback_query_id=cb.get("id"), text="Biraz yavaş 🙂")
                    continue
                if trial_over(state, chat["id"]) and str(chat["id"]) != OWNER_CHAT_ID:
                    tg("answerCallbackQuery", callback_query_id=cb.get("id"), text="Deneme süresi doldu")
                    continue
            ...
            if rate_limited(chat["id"]):
                continue
            if trial_over(state, chat["id"]) and str(chat["id"]) != OWNER_CHAT_ID:
                send(chat["id"], f"⏳ <b>Deneme süresi doldu.</b>{_contact()}")
                continue
...
def maybe_send_summaries(state):
    ...
    due = [cid for cid, s in state["summaries"].items()
           if s.get("last") != today and hhmm >= s.get("time", "99")
           and not (trial_over(state, int(cid)) and cid != OWNER_CHAT_ID)]
    if not due:
        return 0
    text = build_summary()
    for cid in due:
